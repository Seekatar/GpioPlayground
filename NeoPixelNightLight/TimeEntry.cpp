...
int TimeEntry::initialize( int eepromOffset )
{
    ...
    if ( wakeUnixTime >= 0 )
        _wakeTime = time_t(wakeUnixTime);

    Serial.print(F(" which is ") );
    printTime(_wakeTime);
    Serial.println("");

    return eepromOffset + sizeof(time_t);
}
...
bool  TimeEntry::process( bool changingModes )
{
  ...
  if (Serial.available() > 0) {
    ...
    {
      _wakeTime = makeTime(hours,minutes,0,1,1,2016);
      printTime(_wakeTime);
      Serial.print(F(" is new waketime ("));
      Serial.print(_wakeTime);
      Serial.println(F(")"));
      EEPROM.put(_wakeEepromIndex,_wakeTime);
#ifdef ESP8266
      EEPROM.commit();
#endif
    }
